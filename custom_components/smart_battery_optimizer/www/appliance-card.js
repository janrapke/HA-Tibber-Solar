class ApplianceCard extends HTMLElement {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        * { box-sizing: border-box; }
        ...
        .program-name { font-weight: 500; color: var(--primary-text-color); }
        .program-meta { color: var(--secondary-text-color); font-size: 0.93em; }
        ...
        .countdown-wrap { text-align: center; padding: 10px 0 6px; }
        .countdown { font-size: 2.4em; font-weight: 300; color: var(--primary-text-color); letter-spacing: -1px; }
        .countdown-sub { font-size: 0.8em; color: var(--secondary-text-color); margin-top: 2px; }
        ...
        .proposal-wrap { margin-bottom: 12px; }
        select {
          width: 100%;
          padding: 9px 10px;
          border-radius: 8px;
        ...
        .btn-confirm { background: var(--primary-color); color: var(--text-primary-color, #fff); }
        .btn-confirm:disabled { opacity: 0.38; cursor: default; }
      ...
      <ha-card>
        ...
        <div class="body">
          ${isRunning ? this._renderRunning(programs, probs, timerState) : ''}
          ${isWaiting ? this._renderWaiting(plannedStart, estimatedCost) : ''}
          ${!isRunning && !isWaiting ? this._renderIdle(hint, programs, selectOptions, currentOption, hasProposals) : ''}
        </div>
      </ha-card>
    `;

    const sel = this.shadowRoot.querySelector('select');
    if (sel) sel.addEventListener('change', e => this._selectOption(e.target.value));
  ...
  _renderRunning(programs, probs, timerState) {
    ...
    return `
      ${probEntries.length > 0 ? `
        <div class="prob-section">
          <div class="section-label">Programmerkennung</div>
      ...
      ${showTimer ? `
        <div class="countdown-wrap">
          ...
          <div class="countdown-sub">verbleibend</div>
        </div>
      ` : ''}
    `;
  }
  ...
  _renderWaiting(plannedStart, estimatedCost) {
    ...
    return `
      <div class="info-box">
        <div class="info-row">
          <span class="info-label">Geplanter Start</span>
          <span class="info-value">${startDisplay}</span>
        </div>
        ${estimatedCost != null ? `
        <div class="info-row">
          <span class="info-label">Geschätzte Kosten</span>
          <span class="info-value">${estimatedCost} ct</span>
        </div>` : ''}
      </div>
      <div class="actions">
  ...
  _renderIdle(hint, programs, selectOptions, currentOption, hasProposals) {
    const programsHtml = programs.length > 0 ? `
      <div class="programs">
        <div class="section-label">Gelernte Programme</div>
        ${programs.map(p => `
          <div class="program-row">
            <span class="program-name">${p.name}</span>
            <span class="program-meta">${p.dauer_min} min &middot; ⌀&thinsp;${Math.round(p.avg_leistung_w)} W &middot; ${p.laeufe}&times;</span>
          </div>`).join('')}
      </div>
    ...
    if (hint) {
      return `<div class="hint">💡 ${hint}</div>${programsHtml}`;
    }
    ...
    return `
      ...
      ${hasProposals ? `
        <div class="proposal-wrap">
          <div class="section-label">Startvorschlag wählen</div>
          <select>
        ...
        <div class="actions">
          <button class="btn btn-confirm">✓&ensp;Bestätigen</button>
        </div>
      ` : `<div class="hint">Keine Vorschläge — Tibber-Preise oder Gerät prüfen.</div>`}
    `;
  }

  _selectOption(option) {
    if (!this._entities?.select || !this._hass) return;
...
window.customCards.push({
  ...
  description: 'Steuerung und Status für Smart Battery Optimizer Geräte',
});
